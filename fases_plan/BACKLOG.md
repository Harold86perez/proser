...
- [x] La franja superior (hero) de cada página no coincidía con el color de su CTA final — ahora cada página usa un solo color consistente arriba y abajo, según la línea de servicio que representa
- [x] Los colores de marca no eran los oficiales — Harold confirmó los 5 hex exactos por línea de servicio (ver axioma 4 del CLAUDE.md). Se actualizaron las variables de CSS, la banda multicolor, y todos los degradados/valores fijos que quedaban de los colores viejos en `servicios/`, `sobre/`, `proyectos/`, `ebooks/` y `links/`. Recursos/Ebooks pasó de magenta a negro (como Kliné); Membresía ahora combina empresarial+educativo en degradado; Proyectos pasó de verde a amarillo (#f8cc2c, proyección social) con textos oscuros para buen contraste
- [x] Varios elementos con `grid-template-columns` fijo en el HTML (no en una clase) ignoraban las reglas de celular y causaban scroll horizontal — encontrado y corregido en Kliné (planes), Recursos (recursos gratuitos), Membresía (3 pasos) y Proyectos (info de donaciones)
- [x] La foto del equipo en el hero de portada estaba oculta en celular (`display:none`) — ahora se muestra debajo del texto en vez de ocultarse
